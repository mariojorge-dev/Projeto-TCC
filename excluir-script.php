<?php
    ...
    $cpf = $_POST['cod_pessoa'];


    
    $result = mysqli_query($conexao, "DELETE FROM `mydb`.`cliente` WHERE cpf = $cpf");


    if ($result) {
        echo "<script>alert('$nome Excluido com sucesso!');</script>";
        

    } else {
        echo "<script>alert('Não foi possivel excluir $nome!');</script>";
    }
